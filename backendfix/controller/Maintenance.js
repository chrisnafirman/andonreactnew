...
const postRequestMaintenance = (req, res) => {
  const { NamaPIC, Area, Line, Department, Station } = req.body;

  db.query(
    "INSERT INTO maintenance (Nama, Area, Line, Department, Station) VALUES (?, ?, ?, ?, ?)",
    [NamaPIC, Area, Line, Department, Station],
    (error, results) => {
      if (error) {
        console.log(error);
