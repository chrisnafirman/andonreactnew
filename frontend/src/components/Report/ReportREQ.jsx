...
const ReuestLeader = () => {
  ...
  const exportToPDF = () => {
    ...
    const text = "Report Case Leader [REQ]";
    ...
    // Menyimpan file PDF
    doc.save(`Report Case Leader [REQ].pdf`);
  };
  ...
  useEffect(() => {
    ...
    fetch("http://192.168.101.12:3000/api/Repair")
      ...
      .then((json) => {
        ...
        json.forEach((item) => {
          ...
          item.RequestorDepartment = item.Requestor;
    ...
    fetch("http://192.168.101.12:3000/api/leader")
      ...
      .then((json) => {
        ...
        json.forEach((item) => {
          // Sesuaikan properti Leader dengan API Leader Anda
          item.NamaOperator = item.Nama;
  ...
  useEffect(() => {
    // Menyaring data Validation yang memiliki Uid dengan nilai "REQ"
    const filteredValidationData = validationData.filter((validationItem) =>
      validationItem.Uid.includes("REQ")
    );

    // Menyaring data Repair yang memiliki Uid dengan nilai "REQ"
    const filteredRepairData = repairData.filter((repairItem) =>
      repairItem.Uid.includes("REQ")
    );

    // Menyaring data Leader yang memiliki Uid dengan nilai "REQ"
    const filteredLeaderData = leaderData.filter((leaderItem) =>
      leaderItem.Uid.includes("REQ")
    );

    // Menggabungkan data Validation, Repair, dan Leader berdasarkan Uid
    const mergedData = filteredValidationData.map((validationItem) => {
      ...
      const matchingLeaderItem = filteredLeaderData.find(
        (leaderItem) => leaderItem.Uid === validationItem.Uid
      );
      ...
      if (matchingRepairItem && matchingLeaderItem) {
        ...
        return {
          ...
          NamaOperator: matchingLeaderItem.NamaOperator,
          ...
          ValidationDepartment: validationItem.ValidationDepartment,
                ...
                <table id="data-table" className="table-auto w-full">
                  <thead className="text-xs font-mono uppercase text-gray-400 bg-gray-50">
                    <tr>
                      ...
                      <th className="p-1 min-w-[180px] whitespace-no-wrap overflow-x-auto">
                        <div className="text-center flex">Request By</div>
                      </th>
                      <th className="p-1 min-w-[180px] whitespace-no-wrap overflow-x-auto">
                        <div className="text-center flex">Response at</div>
                      </th>
                      <th className="p-1 min-w-[180px] whitespace-no-wrap overflow-x-auto">
                        <div className="text-center flex">Response by</div>
                      </th>
                      <th className="p-1 min-w-[180px] whitespace-no-wrap overflow-x-auto">
                        <div className="text-center flex">Repair To</div>
                      </th>
                      ...
                      <th className="p-1 min-w-[150px] whitespace-no-wrap overflow-x-auto">
                        <div className="text-center flex">Validation To</div>
                      </th>
                      <th className="p-1 min-w-[180px] whitespace-no-wrap overflow-x-auto">
