...
const exportToPDF = () => {
  ...
  doc.save(`Report Damage Status.pdf`); 
};
  ...
  function updateTime() {
    ...
    useEffect(() => {
        fetch("http://localhost:3001/api/get/datakerusakan")
          .then((response) => response.json())
          .then((json) => {
            const RedViewData = json.filter(
              (item) => item.status2 === "Damage"
            );
            // mengubah properti timestamp menjadi tanggal saja
            RedViewData.forEach((item) => {
              const date = new Date(item.timestamp2);
              const formattedDate = `${date.getDate()}/${
                date.getMonth() + 1
              }/${date.getFullYear()}`;
              item.timestamp2 = formattedDate;
            });
            RedViewData.sort(
              (a, b) => Date.parse(a.timestamp2) - Date.parse(b.timestamp2)
            );
            RedViewData.reverse();
            setData(RedViewData);
            setFilteredData(RedViewData);
          });
      }, []);
    ...
    const handleFilterByDate = (e) => {
      ...
      fetch(`http://localhost:3001/api/get/datakerusakan?datekerusakan=${selectedDate}`)
        .then((response) => response.json())
        .then((json) => {
          const RediViewData = json.filter(
            (item) => item.status2 === "Damage"
          );
          // mengubah properti timestamp menjadi tanggal saja
          RediViewData.forEach((item) => {
            const date = new Date(item.timestamp2);
            const formattedDate = `${date.getDate()}/${
              date.getMonth() + 1
            }/${date.getFullYear()}`;
          ...
          RediViewData.reverse();
          setFilteredData(RediViewData);
        });
    };
    ...
    const handleSubmit = (event) => {
      ...
      const filteredData = data.filter(
        (item) => item.timestamp2 === formattedDate
      );
      filteredData.sort(
        (a, b) => Date.parse(b.timestamp2) - Date.parse(a.timestamp2)
      );
    ...
    <body style={styles}>
      <nav class="bg-white px-2 sm:px-4 py-2.5 dark:bg-gray-900  w-full sticky z-20 top-0 left-0 border-b border-gray-200 dark:border-gray-600">
        <div class="container flex flex-wrap items-center justify-between mx-auto">
          ...
          <h1 class="text-center sm:text-left">Report Damage Machine</h1>
                ...
                <table id="data-table" className="table-auto w-full">
                  <thead className="text-xs font-semibold uppercase text-gray-400 bg-gray-50">
                    <tr>
                      ...
                      <th className="p-2">
                        <div className="font-semibold text-center">Problem</div>
                      </th>
                    </tr>
                  </thead>
