...
function QROperator() {
    ...
    const [Line, setLine] = useState("Not Defined");
    const [Area, setArea] = useState("Not Defined");
    ...
    useEffect(() => {
        ...
        const ref10 = db.ref("/StatusLine/SMTLine1CMAOnGoing");
    ...
    const submitLeader = () => {
        ...
        db.ref(`${AreaFirebase}/${Station}`).set("Leader");
    ...
    const submitMaintenance = () => {
        if (!NamaPIC || !Area || !Department || !Station || !Kerusakan || !Sid) {
            alert("Harap isi semua kolom!");
            return;
        }
        ...
        db.ref(`${AreaFirebase}/${Station}`).set("MAINTENANCE & IT");
        db.ref(`StatusLine/${LineFirebase}`).set("Down");
    ...
    const handleScanSuccessMesin = async (data) => {
        try {
            ...
            if (matchedStation) {
                ...
                setArea(matchedStation.Area_Station);
                setLine(matchedStation.Line_Station);
                setStation(matchedStation.Name_Station);
                setSid(data);
                setStationView(matchedStation.Name_Station);
                setShowPopupMesin(false);
                setIsQROperator(true);
            } else {
                            ...
                            <div className="flex items-center justify-center mx-auto">
                                <div className="flex-shrink-0">
                                    <img
                                        ...
                                        alt="Logo"
                                        className="h-20 w-auto ml-4 mt-10  bg-white rounded-md object-cover"
                                    />
                                </div>
                            </div>
