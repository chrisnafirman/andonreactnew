...
const PutDownTimeDestackerTOP = (req, res) => {
  ...
  const PutDownTimeDestackerTOPReturn = (req, res) => {
    const { TimeDestackerTop, DestackerTop, Area } = req.body;
...
const getDestackerTOPValidationReturn = (req, res) => {
    const sqlSelect = "SELECT * FROM returnvalidation WHERE Station = 'Destacker (TOP)' AND Line = 'SMT LINE 1' AND Area = 'SMT TOP' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
const getDestackerTOPReturnRepair = (req, res) => {
    const sqlSelect = "SELECT * FROM returnrepair WHERE Station = 'Destacker (TOP)' AND Line = 'SMT LINE 1' AND Area = 'SMT TOP' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
module.exports = {
    ...
    getDestackerTOPReturnRepair,
    PutDownTimeDestackerTOPReturn,
    getDestackerTOPValidationReturn,
};