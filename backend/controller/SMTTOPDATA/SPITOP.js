...
const PutDownTimeSPITOP = (req, res) => {
  ...
  const PutDownTimeSPITOPReturn = (req, res) => {
    const { TimeSPITop, SPITop, Area } = req.body;
...
const getSPITOPValidationReturn = (req, res) => {
    const sqlSelect = "SELECT * FROM returnvalidation WHERE Station = 'SPI (TOP)' AND Line = 'SMT LINE 1' AND Area = 'SMT TOP' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
module.exports = {
    ...
    getSPITOPValidationReturn,
    PutDownTimeSPITOPReturn,
};