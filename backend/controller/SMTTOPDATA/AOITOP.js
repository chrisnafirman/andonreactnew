...
const PutDownTimeAOITOP = (req, res) => {
  ...
  const PutDownTimeAOITOPReturn = (req, res) => {
    const { TimeAOITop, AOITop, Area } = req.body;
...
const getAOITOPValidationReturn = (req, res) => {
    const sqlSelect = "SELECT * FROM returnvalidation WHERE Station = 'AOI (TOP)' AND Line = 'SMT LINE 1' AND Area = 'SMT TOP' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
module.exports = {
    ...
    PutDownTimeAOITOPReturn,
    getAOITOPValidationReturn,
};