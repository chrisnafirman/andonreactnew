...
const PutDownTimePrinterTOP = (req, res) => {
  ...
  const PutDownTimePrinterTOPReturn = (req, res) => {
    const { TimePrinterTop, PrinterTop, Area } = req.body;
...
const getPrinterTOPValidationReturn = (req, res) => {
    const sqlSelect = "SELECT * FROM returnvalidation WHERE Station = 'Printer (TOP)' AND Line = 'SMT LINE 1' AND Area = 'SMT TOP' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
module.exports = {
    ...
    PutDownTimePrinterTOPReturn,
    getPrinterTOPValidationReturn,
};