...
const PutDownTimeDropinBE = (req, res) => {
  ...
  const PutDownTimeDropinBEReturn = (req, res) => {
    const { TimeDropinBe, DropinBe, Area } = req.body;
...
const getDropinBEValidationReturn = (req, res) => {
    const sqlSelect = "SELECT * FROM returnvalidation WHERE Station = 'Drop in (BE)' AND Line = 'SMT LINE 1' AND Area = 'SMT BE' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
const getDropinBEReturnRepair = (req, res) => {
    const sqlSelect = "SELECT * FROM returnrepair WHERE Station = 'Drop in (BE)' AND Line = 'SMT LINE 1' AND Area = 'SMT BE' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
module.exports = {
    ...
    getDropinBEReturnRepair,
    PutDownTimeDropinBEReturn,
    getDropinBEValidationReturn,
};