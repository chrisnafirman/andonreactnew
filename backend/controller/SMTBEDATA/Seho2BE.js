...
const PutDownTimeSeho2BE = (req, res) => {
  ...
  const PutDownTimeSeho2BEReturn = (req, res) => {
    const { TimeSeho2Be, Seho2Be, Area } = req.body;
...
const getSeho2BEValidationReturn = (req, res) => {
    const sqlSelect = "SELECT * FROM returnvalidation WHERE Station = 'Seho2 (BE)' AND Line = 'SMT LINE 1' AND Area = 'SMT BE' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
const getSeho2BEReturnRepair = (req, res) => {
    const sqlSelect = "SELECT * FROM returnrepair WHERE Station = 'Seho2 (BE)' AND Line = 'SMT LINE 1' AND Area = 'SMT BE' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
module.exports = {
    ...
    getSeho2BEReturnRepair,
    PutDownTimeSeho2BEReturn,
    getSeho2BEValidationReturn,
};