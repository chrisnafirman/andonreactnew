...
const PutDownTimeTouchupBE = (req, res) => {
  ...
  const PutDownTimeTouchupBEReturn = (req, res) => {
    const { TimeTouchupBe, TouchupBe, Area } = req.body;
...
const getTouchupBEValidationReturn = (req, res) => {
    const sqlSelect = "SELECT * FROM returnvalidation WHERE Station = 'Touch UP (BE)' AND Line = 'SMT LINE 1' AND Area = 'SMT BE' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
const getTouchupBEReturnRepair = (req, res) => {
    const sqlSelect = "SELECT * FROM returnrepair WHERE Station = 'Touch UP (BE)' AND Line = 'SMT LINE 1' AND Area = 'SMT BE' ORDER BY No DESC LIMIT 1";
    db.query(sqlSelect, (err, results) => {
        if (err) {
            console.log(err);
            res.status(500).send("Error fetching data from the database.");
...
module.exports = {
    ...
    getTouchupBEReturnRepair,
    PutDownTimeTouchupBEReturn,
    getTouchupBEValidationReturn,
};